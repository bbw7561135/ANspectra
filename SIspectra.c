...
void SIspectra(int nt, int nf){
	...
	const char t[cht]={'n','a'}, f[chf]={'e','m'};
	const char *prefix="./data/", *postfix="E2.dat", *siname="HGm_KM";
	char sifile[256],pict[256];
	FILE *fsi=101;
	double sp[che][cha],y;
	int ed, e, a, ndr;
	TCanvas csp("csp","Spectra",200,200), cza("cza","Zenith Angle Distribution",200,200);
	TGraph grsp[cha-1],grza[ched];

	SetStyle();
//	sprintf(sifile,"%s%s_%c%c_%s",prefix,siname,t[nt],f[nf],postfix);
	sprintf(sifile,"./output/H11xsp%c%c.dat",f[nf],t[nt]);
	fsi=fopen(sifile,"r");
//	sp[0][0]=0;
	for(a=0; a<cha; a++){
//		sp[0][a]=1-0.1*(a-1);
	...
	for(ndr=0; ndr<2; ndr++){
		...
		csp.SetLogx();
		csp.SetLogy();
		sprintf(pict,"./pictures/%s_%c%c_spectra.eps",siname,t[nt],f[nf]);
		csp.Print(pict);
		delete legsp;
	}
...
//	TLegend* legza=new TLegend(0.5,0.3,0.85,0.75);
	...
	legza->Draw();
	sprintf(pict,"./pictures/%s_%c%c_ZAD.eps",siname,t[nt],f[nf]);
	cza.Print(pict);
	delete legza;
}
